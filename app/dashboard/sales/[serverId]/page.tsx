      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[#6b6b8a] text-sm">Recent transactions for <span className="text-white">{server.name}</span></p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-[#111118] border border-[#1e1e30] rounded-2xl p-5">
            <p className="text-[#6b6b8a] text-xs uppercase font-medium mb-1">Total Transactions</p>
            <p className="text-2xl font-bold text-white">{transactions.length}</p>
          </div>
          <div className="bg-[#111118] border border-[#1e1e30] rounded-2xl p-5">
            <p className="text-[#6b6b8a] text-xs uppercase font-medium mb-1">Completed</p>
            <p className="text-2xl font-bold text-green-400">{completedCount}</p>
          </div>
          <div className="bg-[#111118] border border-[#1e1e30] rounded-2xl p-5">
            <p className="text-[#6b6b8a] text-xs uppercase font-medium mb-1">Revenue (completed)</p>
            <p className="text-2xl font-bold text-[#7c6af7]">
              {server.defaultCurrency?.toUpperCase()} {(totalRevenue / 100).toFixed(2)}
            </p>
          </div>
        </div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-[#1e1e30]/50 text-[#6b6b8a] uppercase text-xs">
                  <tr>
                    ...
                    <th className="px-5 py-4 font-medium">Cash</th>
                ...
                <tbody className="divide-y divide-[#1e1e30]">
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-[#1a1a24] transition-colors">
                      <td className="px-5 py-4 text-[#8b8bAA] whitespace-nowrap">
                        {new Date(tx.createdAt).toLocaleString()}
                      </td>
                      <td className="px-5 py-4 text-white font-medium">
                        {tx.playerName}
                      </td>
                      <td className="px-5 py-4 text-[#6b6b8a] font-mono text-xs">
                        {tx.playerIp || "—"}
                      </td>
                      <td className="px-5 py-4 text-[#7c6af7] font-semibold">
                        {tx.amount} Cash
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span className="px-2 py-0.5 rounded-md bg-[#1e1e30] text-[#8b8bAA] text-xs font-mono uppercase">
